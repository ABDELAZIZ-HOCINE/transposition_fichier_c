    ...
    while (j < k)
    {
        for (int i = 0; i < k; i++) {
            if (mdp_t[j] != mdp[i] || indices_traites[i]) {
                ...
                sprintf(subfilename, "%s-KEY-%s-PART-%d",mdp, file, j + 1);
...
int count_file_with_prefix(const char* password, const char* prefix) {
    const char* directory_path = ".";  // Chemin vers le répertoire courant
...
char* extract_password_from_filename(const char* prefix) {
...
int reconstruire_depuis_matrice(char** tab_transpose, int nb_lignes, int nb_colonnes, const char* password,const char* output_filename) {
    ...
    DIR* dir = opendir(".");  // Ouvrir le répertoire courant
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        if (strncmp(entry->d_name, prefix, strlen(prefix)) == 0) {
            snprintf(filepath, sizeof(filepath), "./%s", entry->d_name);  // Construire le chemin complet du fichier
...
// Fonction pour lire un fichier, remplir et transposer le tableau
int detranspose(char* filename, char* mdp) {
    ...
    int nb_subfile = count_file_with_prefix(mdp,prefix);  // Compte le nombre de fichiers avec ce préfixe
    ...
    int nb_lignes = size_largest_file(".",mdp,prefix);
    //printf("nb_lignes: %d\n",nb_lignes);
    ...
    while (j < k) {
        for (int i = 0; i < k; i++) {
            ...
            if (mdp_t[j] != mdp[i] || indices_traites[i]) {
                ...
                FILE* file = fopen(subfile, "rb");  // Ouvre le fichier en lecture binaire
                if (!file) {
    ...
    reconstruire_depuis_matrice(tab_transpose, nb_lignes, k,mdp, filename);
...
int main(int argc, char* argv[]) {
    ...
    if (argc == 4 && strcmp(argv[1], "-d") == 0) {
        char* filename = argv[2];  // Le nom du fichier à reconstruire
        char * my_pass= argv[3];

        //extraction de mot de passe a partir des fichier trouvés
        char* password = extract_password_from_filename(filename);
        ...
        int result = detranspose(filename, password);
    ...
    // Cas de la transposition (diviser un fichier en plusieurs fichiers enfants)
        ...
        char* filename = argv[1];  // Le fichier à transposer
